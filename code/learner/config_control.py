...
from code.common import singleton
...
class ConfigControl(object):
    def __init__(self, config_file="code/common.conf"):
        ...
        self.save_model_seconds = config.getint(
            "main", "save_model_seconds", fallback=2 * 60
        )
        self.save_model_dir = (
        ...
        self.display_every_second = config.getint(
            "main", "display_every_second", fallback=2 * 60
        )
        ...
        self.distributed_backend = config.get(
            "main", "distributed_backend", fallback=None
        )
        ...
        self.dump_profile = config.getboolean(
            "main", "dump_profile", fallback=False
        )  # torch-only

        self.use_init_model = config.getboolean("model", "use_init_model")
        self.load_optimizer_state = config.getboolean(
            "model", "load_optimizer_state", fallback=True
        )
        ...
        self.use_jit = config.getboolean(
            "model", "use_jit", fallback=False
        )  # torch-only
        self.use_compile = config.getboolean(
            "model", "use_compile", fallback=False
        )  # torch-only
        self.use_mix_precision = config.getboolean("model", "use_mix_precision")
        self.channels_last = config.getboolean(
            "model", "channels_last", fallback=False
        )  # torch-only
        self.use_fp16 = (
        ...
        self.has_unused_params = config.getboolean(
            "model", "has_unused_params", fallback=False
        )  # torch ddp-only
